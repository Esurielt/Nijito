...
{
    public static class BeatmapFileUtility
    {
        /*
        ...
        public static bool CreateSongDataDirectory()
        {
            ...
            {
                Game.LogFormat(Logging.Category.BEATMAP, "Failed to create main song data directory: {0}", Logging.Level.LOG_ERROR, e);
                return false;
            }
        }
        public static bool CreateNewSong(string songName)
        {
            ...
            try
            {
                ...
                // Need to make a SerializableSongMetadata class

                return true;
            }
            catch (System.Exception e)
            {
                Game.LogFormat(Logging.Category.BEATMAP, "Failed to create song directory: {0}", Logging.Level.LOG_ERROR, e);
                return false;
            }
        }
                    ...
                    if (File.Exists(filePath))
                    {
                        ...
                        if (TryDeserializeBeatmap(bytes, out SerializableBeatmap beatmapData))
                        {
                            if (beatmapData.TypeName == typeInstance.Name)
                            {
                                ...
                                if (newBeatmap != null)
                                {
                                    Game.Log(Logging.Category.BEATMAP, "Read beatmap from file: " + filePath, Logging.Level.LOG);
                                    return true;
                                }
        ...
        public static bool TryWriteBeatmap(string songName, BeatmapDifficulty difficulty, IBeatmap beatmap)
        {
            try
            {
                ...
                Game.Log(Logging.Category.BEATMAP, "Wrote beatmap to file: " + directory, Logging.Level.LOG);
                return true;
            }
            catch (System.Exception e)
            {
                Game.Log(Logging.Category.BEATMAP, "Unable to write beatmap to file: " + e, Logging.Level.LOG_WARNING);
            }
            return false;
        }
        ...
        public static bool TryGetAudio(string songName, out AudioClip audio)
        {
            ...
            try
            {
                ...
                if (Directory.Exists(dirPath))
                {
                    ...
                    if (filePaths.Length > 0)
                    {
                        ...
                        if (audio != null)
                        {
                            Game.Log(Logging.Category.BEATMAP, "Read audio from file: " + filePath, Logging.Level.LOG);
                            return true;
                        }
            ...
            {
                Game.Log(Logging.Category.BEATMAP, "Unable to read audio from file: " + e, Logging.Level.LOG_WARNING);
                return false;
            }
        }
        public static bool TryImportAudio(string songName, string sourceFilePath)
        {
                    ...
                    if (fileBytes.Length > 0)
                    {
                        if (Etoile.OpenWavParser.IsCompatible(fileBytes))
                        {
                            if (Etoile.OpenWavParser.FormatIsValid(fileBytes))
                            {
                                ...
                                Game.Log(Logging.Category.BEATMAP, "Successfully imported audio.");
                                return true;
                            }
            ...
            {
                Game.Log(Logging.Category.BEATMAP, $"Unable to import audio: {e}", Logging.Level.LOG_WARNING);
                // Fall out
            }
        ...
        public static bool TryGetSongMetadata(string songName, out SongMetadata metadata)
        {
            ...
            try
            {
                ...
                if (Directory.Exists(dirPath))
                {
                    ...
                    if (File.Exists(filePath))
                    {
                        ...
                        if (metadata != null)
                        {
                            Game.Log(Logging.Category.BEATMAP, "Read song metadata from file: " + filePath, Logging.Level.LOG);
                            return true;
                        }
            ...
            {
                Game.Log(Logging.Category.BEATMAP, "Unable to read song metadata from file: " + e, Logging.Level.LOG_WARNING);
                return false;
            }
        }
        ...
        public static bool TryWriteSongMetadata(string songName, SongMetadata metadata)
        {
            try
            {
                string jsonStr = JsonUtility.ToJson(metadata);
                ...
                Game.Log(Logging.Category.BEATMAP, "Wrote song metadata to file: " + filePath, Logging.Level.LOG);
                return true;
            }
            catch (System.Exception e)
            {
                Game.Log(Logging.Category.BEATMAP, "Unable to write song metadata to file: " + e, Logging.Level.LOG_WARNING);
            }
            return false;
        }
        ...
        public static bool TryGetAudioMetadata(string songName, out AudioMetadata metadata)
        {
            ...
            try
            {
                ...
                if (Directory.Exists(dirPath))
                {
                    ...
                    if (File.Exists(filePath))
                    {
                        ...
                        if (serialized != null)
                        {
                            metadata = new AudioMetadata(serialized);
                            Game.Log(Logging.Category.BEATMAP, "Read audio metadata from file: " + filePath, Logging.Level.LOG);
                            return true;
                        }
            ...
            {
                Game.Log(Logging.Category.BEATMAP, "Unable to read audio metadata from file: " + e, Logging.Level.LOG_WARNING);
                return false;
            }
        }
        ...
        public static bool TryWriteAudioMetadata(string songName, AudioMetadata metadata)
        {
            try
            {
                ...
                Game.Log(Logging.Category.BEATMAP, "Wrote audio metadata to file: " + filePath, Logging.Level.LOG);
                return true;
            }
            catch (System.Exception e)
            {
                Game.Log(Logging.Category.BEATMAP, "Unable to write audio metadata to file: " + e, Logging.Level.LOG_WARNING);
            }
            return false;
        }
    }
