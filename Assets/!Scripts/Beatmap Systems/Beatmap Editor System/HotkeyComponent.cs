...
namespace Beatmap.Editor
{
    public class HotkeyComponent : EditorComponent
    {
        ...
        public void RegisterHotkey(string functionName, System.Action action, KeyCombo keyCombo)
        {
            ...
            foreach (var toggleKey in ToggleKey.GetAll())
            {
                ...
                {
                    Game.Log(Logging.Category.BEATMAP, $"Unable to register hotkey: Main key (Keycode.{keyCombo.MainKey}) is a reserved toggle key.",
            ...
            if (_hotKeyRegistry.Exists(kvp => kvp.Key.Equals(keyCombo)))
            {
                ...
                Game.LogFormat(Logging.Category.BEATMAP,
