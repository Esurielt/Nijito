...
namespace Beatmap.Editor
{
    ...
    public class AudioComponent : EditorComponent
    {
        ...
        {
            AudioClip audio = Editor.IOComponent.TryLoadData<AudioClip>(BeatmapFileUtility.TryGetAudio, "song audio");
            SetAudio(audio);
        ...
        public void SetAudio(AudioClip audioClip)
        {
            ...
            Game.Log(Logging.Category.BEATMAP, "Beatmap editor audio set.", Logging.Level.LOG);
            if (audioClip == null)
                Game.Log(Logging.Category.BEATMAP, "(Beatmap editor audio is null)", Logging.Level.LOG);
        }
        public void Play()
        {
            if (HasAudio)
        ...
        {
            if (BeatmapFileUtility.TryImportAudio(Editor.SongName, filePath))
            {
