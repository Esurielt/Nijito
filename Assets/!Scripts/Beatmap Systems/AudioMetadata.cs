...
namespace Beatmap
{
    ...
    public class AudioMetadata
    {
        ...
        public AudioMetadata(Serialization.SerializableAudioMetadata serializable)
        {
            _startingDelay = serializable.StartingDelay;
            BpmChanges = serializable.BpmChanges.GetAsValueHelper();
            SectionNames = serializable.SectionNames.GetAsValueHelper();
        }
        public class ValueHelper<TValue>
        {
            ...
            public Dictionary<int, TValue> GetValueDictCopy()
            {
            ...
            public bool TryAddValue(int atBeatIndex, TValue newValue)
            {
                ...
                {
                    Game.LogFormat(Logging.Category.BEATMAP, "Value helper already has a value at index {0}", Logging.Level.LOG_WARNING, atBeatIndex);
                    return false;
                }
            }
            public bool TrySetValue(int atBeatIndex, TValue newValue)
            {
                ...
                {
                    Game.LogFormat(Logging.Category.BEATMAP, "Value helper does not have a value at index {0}", Logging.Level.LOG_WARNING, atBeatIndex);
                    return false;
                }
            }
