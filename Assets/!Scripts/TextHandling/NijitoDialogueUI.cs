...
/*
All of this was yoinked from the Yarn Spinner GitHub, specifically the
Yarn.Unity.DialogueUI object. You can find that here:
 https://github.com/YarnSpinnerTool/YarnSpinner-Unity/blob/master/Runtime/DialogueUI.cs

This is obviously not ideal, but their coding style necessitated it.
The problem was that we want the speaker's name to preceed the text,
but we'd also like to keep all of the other nice features that come
with their DialogueUI class... why throw all of that away?

All of the other code is locked in a DLL... this is the first place
the text is actually pulled and used, and we can't intercept it
anywhere.

We can't send the text off to another object to figure out the name;
this is in charge of writing out the text itself, so the full name
isn't immediately available like we want it to. We *could* pull the
writing code and put it in another object, but then we're essentially
just recreating that part of the code. (Though we may be doing this
anyway? We'll see.)

Nor could we just inherit from DialogueUI and modify the behavior;
the part where the text is pulled out is buried deep within a private
method. Trying to do that in a child object would ALSO require copying
and pasting.

So, no matter what we do, we'll probably be copy/pasting to some extent.
Just copying it all gets us a baseline to start with, but we have full
control over how it behaves, which is exactly what we want.

Unless we drastically overhaul this class, any small deviations from the
original class will be marked with a MOD comment.
*/
...
/// <summary>
/// This namespace contains all of the code for Nijito's dialogue systems.
/// 
/// Our dialogue system builds on Yarn spinner. You can refer to the
/// documentation for [Yarn Spinner](https://yarnspinner.dev/docs/tutorial)
...
///
/// Note that the leading colon on the third line is OPTIONAL.
/// Leaving it in, however, ensures that the game doesn't get confused
/// if you have a colon somewhere in your text.
/// </summary>
namespace Dialogue
{
	...
	public class NijitoDialogueUI : Yarn.Unity.DialogueUIBehaviour
	{
		...
		[Tooltip(
			"This gets called whenever the speaker changes." +
			"This does not get called between two messages if the speaker is the same." +
			"If there is no speaker, onNoSpeaker gets called instead."
		)]
		public DialogueRunner.StringUnityEvent onSpeakerUpdate; // MOD

		[Tooltip(
			"This only gets called when there is no speaker at all." +
			"(i.e. Message has nothing before ':' or is missing ':' entirely.)"
		)]
		public UnityEngine.Events.UnityEvent onNoSpeaker; // MOD
		...
		private string currentSpeaker = ""; // MOD
