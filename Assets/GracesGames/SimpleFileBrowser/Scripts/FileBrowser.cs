...
// [Kodakami was here and oiled up some rusty Unity interaction ^.~]

namespace GracesGames.SimpleFileBrowser.Scripts {

	// [Added actual UnityEvent. ~K]
	...
	public class FileBrowser : MonoBehaviour {
		...
		public UserInterface FileBrowserLandscapeUiPrefab;

		// The file browser UI Portrait mode as prefab
		public UserInterface FileBrowserPortraitUiPrefab;
		...
		// Unity Action Event for closing the file browser [changed to actual UnityEvent ~K]
		[HideInInspector] public UnityEvent OnFileBrowserClose;

		// Unity Action Event for selecting a file [changed to actual UnityEvent subclass ~K]
		[HideInInspector] public FileSelectedEvent OnFileSelect;

		// [Added callback to set up UnityEvents. ~K]
		private void Awake()
		{
			OnFileBrowserClose = new UnityEvent();
			OnFileSelect = new FileSelectedEvent();
		}
		...
		// [Replaced rigid GameObject.Find() use. Pared down unneccessary GameObject use. ~K]
		public void SetupFileBrowser(ViewMode newViewMode, GameObject canvasContainer, string startPath = "") {
			// Set the view mode (landscape or portrait)
			ViewMode = newViewMode;

			// Find the canvas so UI elements can be added to it
			GameObject uiCanvas = canvasContainer;
			...
			if (uiCanvas != null) {
				UserInterface userInterfacePrefab =
					ViewMode == ViewMode.Portrait ? FileBrowserPortraitUiPrefab : FileBrowserLandscapeUiPrefab;
				_uiScript = Instantiate(userInterfacePrefab, uiCanvas.transform);
		...
		// [Directly destroyed GameObject (no need to use GameObject.Find()). Removed silly self-destructive code. Automatically unsubscribed all event listeners. ~K]
		private void Destroy() {
			// Set _isOpen
			_isOpen = false;

			OnFileBrowserClose.RemoveAllListeners();
			OnFileSelect.RemoveAllListeners();
			Destroy(_uiScript.gameObject);
		}
	}
}